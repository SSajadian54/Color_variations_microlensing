...
///=================================================
   ...
   FILE* magni; 
   FILE* limbd; 
   param=fopen("./files/LimbChorB_all.txt","w");
   fclose(param); 
   ...
   double t, u, mmag;
   double xlens, ylens, delVI, delBR,  delVI0, delBR0;
   ...
   l.dt=double(1.0/60.0/24.0);
   CauCross(l,icon); 
   int Nlens=int((l.pt2-l.pt1)*1.00/l.dt +3.00); 
   cout<<"Nlens:  "<<Nlens<<endl;
   cout<<"tE:     "<<l.tE<<"\t tstar:  "<<l.tstar<<endl;
   cout<<"Nlens:   "<<Nlens<<"\t l.dt(min):  "<<l.dt*24.0*60.0<<endl;
   ...
   double *magU0=new double[Nlens];
   double *magB0=new double[Nlens];
   double *magV0=new double[Nlens];
   double *magR0=new double[Nlens];
   double *magI0=new double[Nlens];
    ...
    if(i==0) magU[k] = s.blend[0]* mmag + 1.0-s.blend[0];///U
    if(i==1) magB[k] = s.blend[1]* mmag + 1.0-s.blend[1];///B
    if(i==2) magV[k] = s.blend[2]* mmag + 1.0-s.blend[2];///V
    if(i==3) magR[k] = s.blend[3]* mmag + 1.0-s.blend[3];///R
    if(i==4) magI[k] = s.blend[4]* mmag + 1.0-s.blend[4];///I
    } cout<<"filter:  "<<i<<endl; }
    ...
    u = sqrt(xlens*xlens + ylens*ylens);
    mmag=vbb.BinaryMag2(l.dis,l.q, xlens, ylens ,s.ro_star);
    if(i==0) magU0[k] = s.blend[0]* mmag + 1.0-s.blend[0];///U
    if(i==1) magB0[k] = s.blend[1]* mmag + 1.0-s.blend[1];///B
    if(i==2) magV0[k] = s.blend[2]* mmag + 1.0-s.blend[2];///V
    if(i==3) magR0[k] = s.blend[3]* mmag + 1.0-s.blend[3];///R
    if(i==4) magI0[k] = s.blend[4]* mmag + 1.0-s.blend[4];///I
    } cout<<"filter:  "<<i<<endl; }
    ...
    if(icon%1==0) {
    flagff=1; 
    sprintf(filename1,"./files/MONTLimbB/%c%c%d.dat",'M','_',icon);
    magni=fopen(filename1,"w");
    sprintf(filename2,"./files/MONTLimbB/%c%c%d.dat",'D','_',icon);
    danish=fopen(filename2,"w"); 
    ///################################################################################
    sprintf(filename3,"./files/MONTLimbB/%c%c%d.dat",'S','_',icon);
    ...
    tim=0.0;
    chi0=chi1=0.0;  
    int ndat=0; morn=fabs((double)rand()*24.0/(double)(RAND_MAX+1.)); 
    MV=MI=col=col0=0.0;
    ...
    xlens = (t-l.t0)/l.tE * cos(l.ksi) - l.u0 * sin(l.ksi);
    ylens = (t-l.t0)/l.tE * sin(l.ksi) + l.u0 * cos(l.ksi);
    cade= 40.0*sqrt(3.0/sqrt(5.0)/fabs(magI[i]))/60.0;///hour    
    ...
    col = col0+ ttr* sigc;
    deltaA= fabs(pow(10.0,-0.4*er.errI)-1.0)*magI[i]; ///error in magnification
    ...
    delVI= -2.5*log10(double(magV[i])/double(magI[i]))*1000.0;
    delBR= -2.5*log10(double(magB[i])/double(magR[i]))*1000.0;   
    
    delVI0=-2.5*log10(double(magV0[i])/double(magI0[i]))*1000.0;//mmag
    delBR0=-2.5*log10(double(magB0[i])/double(magR0[i]))*1000.0;//mmag 
    
    if(flagff>0) fprintf(magni,"%.4lf  %.5lf   %.5lf    %.5lf   %.5lf  %.5lf    %.5lf    %.5lf  %.8lf  %.8lf   %.8lf  %.8lf\n",
    (t-l.t0)/l.tstar,xlens,ylens,magU[i],magB[i],magV[i],magR[i],magI[i],delVI,delBR, delVI0,delBR0);
    ...
    param=fopen("./files/LimbChorB_all.txt","a+");
    fprintf(param,"%d  %d  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.7lf   %.5lf  %.5lf  %d  %d  %d  %.5lf  %.5lf  %.5lf    %.5lf   %.5lf  %.5lf  %.8lf  %.5lf  %.5lf  %.5lf  %.5lf  %d   %d  %.5lf  %.5lf   %.5lf  %.5lf  %.5lf  %.5lf   %.5lf\n",
    l.numl,l.struc,l.Ml,l.Dl,l.xls,l.RE/Au,l.Vt,l.vs,l.vl,l.tE,l.u0/s.ro_star,l.ksi,l.tstar,  //13 
    s.nums,s.struc,s.cl,s.mass,s.Ds,s.logg,s.logl,s.Teff,s.Rs,s.ro_star,s.magb[4],s.Map[4],s.blend[4],dchi,icon,ndat, l.q, l.dis,//31
    s.slop,deltt,cVI, cBR, Amax);//36
    fclose(param);
    cout<<"******************************************************"<<endl;
    ...
    delete [] magU0, magB0, magV0, magR0, magI0;
    }//end of loop
    }
...
void CauCross(lens & l, int icon){
   ...
   FILE *caustic;
   char filnam1[40];
   sprintf(filnam1,"./files/MONT/%c%c%c%c%d.dat",'c','a','u','_',icon);
...
void ErrorCal(error & er, double mgR, double mgV, double mgI, double slope, double  ratio){
   ...
   er.errI=sqrt(dVI*dVI-er.errV*er.errV);
   if( (ratio>1.0 and er.errV<er.errI) or (ratio<1.0 and er.errI<er.errV)){
...
void func_source(source & s, CMD & cm, extinc & ex)
{
   ...
   double Mab[M]; 
    ...
    if(s.type>8.0 or s.type<2.0 or s.Rs<0.0 or s.Rs>1000.0 or s.mass<0.0 or s.Teff<0.0 or s.cl==6 or s.logg<0.0 or s.cl<0  or s.cl>9 or s.slop<1.5 or s.slop>2.5){
...
void Func_lens(source & s , lens & l)
{
  ...
  l.dis= RandR(0.4,1.4);
    ...
    //l.pt1=-0.6*l.tE;// l.tstar;///days
    //l.pt2=+0.6*l.tE;//l.tstar;///days
