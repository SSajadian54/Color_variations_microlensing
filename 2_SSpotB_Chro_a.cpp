...
///=================================================
   ...
   FILE* magni; 
   FILE* limbd; 
   param=fopen("./files/MONTSpotB/SpotChroB_all_a.txt","w");
   fclose(param); 
   ...
   cout<<"****** End of reading limbD.txt ************"<<endl; 
    ...
    int ll,tt, nfi, LK, ndat;
    char filename1[40], filename2[40], filename3[40];
    double t, u, lonn;
    double mag[M], magt[M], Ftot[M], Fbas[M];
    double xlens,ylens, delVIl, delBRl, delVIls, delBRls;
    double disg, min1, dchi;
    double Fs[M], S[2][M], ff[M], Mu, ux, uy, Magn;
    double x, y, ym, Delta, z, rho, x1, y1, z1, tet, area, rho1;
    double chi0, chi1, morn, weather, tim, cade; 
    double ffe, col, col0, MV, MI; 
    double ttr, deltaA; 
    int Nlens; 
    double SqN_V, SqN_I, ratio, MRR, sigc;  
    double interval,  cVI, cBR, deltt, mmag; 
    int ftime, flagff; 
    double Amax; 
  ...
  /// Nlens= int( (l.pt2-l.pt1)*1.000/l.dt ) + 4;}
   Func_spot(p);
   Delta=double(s.Rs/55.0);
   area = pi*s.Rs*s.Rs;
   rho1= sqrt(Delta*Delta/M_PI)*l.proj;
    ...
    cout<<"Delta: "<<Delta<<"\t area:  "<<area<<"\t rho1:  "<<rho1<<endl;
    cout<<"Teff:  "<<s.Teff<<"\t logg:  "<<s.logg<<"\t Nlens:  "<<Nlens<<endl;
    ...
    if(icon<300){
    flagff=1;
    sprintf(filename1,"./files/MONTSpotB/%c%c%d.dat",'m','_',icon);
    magni=fopen(filename1,"w");
    sprintf(filename2,"./files/MONTSpotB/%c%c%d.dat",'d','_',icon);
    danish=fopen(filename2,"w");
    sprintf(filename3,"./files/MONTSpotB/%c%c%d.dat",'s','_',icon);
    ...
    cout<<"First step:  tem:  "<<tem[ll]<<"\t Teffs: "<<s.Teff<<endl;
    min1=100.0;
    for(int i=0; i<nli; ++i){
    if(tem[i]==tem[ll]){
    disg=fabs(logg[i]-s.logg);
    if(disg<min1){min1=disg;  tt=i;}}}
    nfi= nn[tt];
    double inten[M][nfi];  double mu[nfi], r[nfi];
    for(int i=0; i<nfi; ++i){
    inten[0][i]=inten[1][i]=inten[2][i]=inten[3][i]=inten[4][i]=mu[i]=r[i]=0.0; }
    cout<<"Second step:  "<<tt<<"\t logg:  "<<logg[tt]<<"\t logg_source:  "<<s.logg<<endl;
    cout<<"nn[tt]:  "<<nn[tt]<<"\t Teff:  "<<s.Teff<<"\t tem: "<<tem[tt]<<endl;
    ...
    FILE* inte;
    sprintf(filename3,"./files/filter_It/%d%c%.1f.txt",int(tem[tt]),'_',float(logg[tt]));
    cout<<"tem: "<<tem[tt]<<"\t logg: "<<float(logg[tt]*1.0)<<endl;
    inte=fopen(filename3,"r");
    if(!inte){cout<<"Can not open file in filter_It:  "<<endl;  exit(0);}
    cout<<"End of reading the file "<<endl;
    Fs[0]=Fs[1]=Fs[2]=Fs[3]=Fs[4]=0.0;
    for(int j=0; j<nfi; ++j){
    fscanf(inte,"%lf   %lf   %lf  %lf   %lf   %lf\n",&mu[j],&inten[0][j],&inten[1][j],&inten[2][j], &inten[3][j], &inten[4][j]);    
    if(mu[j]==0.0)       r[j]=fabs(s.Rs);
    else if (mu[j]==1.0) r[j]= 0.0;
    else                 r[j]= s.Rs*sqrt(1.0-mu[j]*mu[j]);
    //cout<<"mu[j]:  "<<mu[j]<<"\t r[j]:  "<<r[j]<<"\t inten[0]: "<<inten[0][j]<<"\t inten[4]:  "<<inten[4][j]<<endl;
    if(j>0){
    for(int i=0; i<M; ++i)  Fs[i]+= 2.0*M_PI*r[j]*fabs(r[j]-r[j-1])*fabs(inten[i][j]);}}
    fclose(inte);
    if(mu[nfi-1]!=1.0  or mu[0]>0.0000001){cout<<"ERROR mu_last:  "<<mu[nfi-1]<<endl;  int dds;  cin>>dds; }
    cout<<"Fs[0]:  "<<Fs[0]<<"\t Fs[1]: "<<Fs[1]<<"\t Fs[2]: "<<Fs[2]<<endl;
    cout<<"Fs[3]:  "<<Fs[3]<<"\t Fs[4]:  "<<Fs[4]<<endl;


    for(int i=0; i<M; ++i){
    vbb.SetLDprofile2(int(tem[tt]), float(logg[tt]), int(nn[tt]), int(i) );
    ...
    mmag=vbb.BinaryMag2(l.dis,l.q, xlens, ylens ,s.ro_star);
    if(i==0) magU[k] =mmag;
    if(i==1) magB[k] =mmag;
    if(i==2) magV[k] =mmag;
    if(i==3) magR[k] =mmag;
    if(i==4) magI[k] =mmag;}  cout<<"filter:  "<<i<<endl; }
    ...
    tim=0.0;
    chi0=chi1=0.0;  
    ndat=0; morn=fabs((double)rand()*24.0/(double)(RAND_MAX+1.)); 
    MV=MI=col=col0=0.0;
    ...
    ylens = (t-l.t0)/l.tE * sin(l.ksi) + l.u0 * cos(l.ksi);
    u = sqrt(xlens*xlens + ylens*ylens);
    for(int i=0; i<M; ++i){ S[0][i]=S[1][i]=0.0;}
///###########################################################################################
    for(x=-s.Rs; x<=s.Rs;   x+=Delta){
    ym=sqrt(fabs(s.Rs*s.Rs-x*x));
    if(fabs(x)==s.Rs) ym=0.0;
    for(y=-ym;      y<=ym;  y+=Delta){
    
    x1= cos(p.tet)*cos(p.fi)*x+cos(p.tet)*sin(p.fi)*y-sin(p.tet)*z;
    y1=-sin(p.fi )*x+cos(p.fi)*y;
    z1= sin(p.tet)*cos(p.fi)*x+sin(p.tet)*sin(p.fi)*y+cos(p.tet)*z;
    p.Rspot=sqrt(x1*x1+y1*y1);
    tet=-1.0;
    tet=asin(fabs(p.Rspot/s.Rs));
    if(p.Rspot==0.0)  tet=0.0;
    if(p.Rspot==s.Rs) tet= M_PI/2.0;
    ...
    Mu=-1;
    if(rho<s.Rs){
    z=sqrt(fabs(s.Rs*s.Rs-rho*rho));
    Mu= sqrt(1.0-rho*rho/(s.Rs*s.Rs));}
    else{z=0.0;     Mu=1.0;}
    LK=-1;
    if(Mu<0.0 or Mu<=mu[0] or Mu==0.0)  LK=0;
    else if(Mu>=mu[nfi-1] or Mu>=1.0 or Mu==1.0) LK= nfi-1;
    else{
    for(int j=1; j<nfi;  ++j){
    if( (Mu-mu[j])*(Mu-mu[j-1])<=0.0 or Mu==float(mu[j-1])){LK=j-1;     break;}}} 
    ///================================================
    if(LK<0 or Mu<0.0 or p.Rspot<0.0 or tet<0.0){
    cout<<"ERROR LK:  "<<LK<<"\t Mu:  "<<Mu<<endl; cout<<"Rspot:  "<<p.Rspot<<"\t tet:  "<<tet<<endl; int uue;  cin>>uue; }
    
    
    ff[0]=ff[1]=ff[2]=ff[3]=ff[4]=0.0;
    if(tet<=p.tet0){    
    ux= xlens -x*l.proj;
    uy= ylens -y*l.proj;
    u= sqrt(ux*ux+ uy*uy);
    Magn=vbb.BinaryMag2(l.dis , l.q, ux,  uy , rho1);
    for(int i=0; i<M; ++i){
    ff[i]=p.frac*inten[2][nfi-1]-inten[i][LK]; 
    S[0][i] += Delta*Delta * ff[i] *  1.0;
    S[1][i] += Delta*Delta * ff[i]* Magn;} }
    if(k==0)   
    if(flagff>0) fprintf(spots,"%.9lf   %.9lf    %.7lf   %.7lf  %.7lf  %.7lf  %.7lf\n",
    double(x/s.Rs),double(y/s.Rs),inten[0][LK]+ff[0],inten[1][LK]+ff[1],inten[2][LK]+ff[2],inten[3][LK]+ff[3],inten[4][LK]+ff[4]);}}

///##################################################################################################
    for(int i=0;  i<M;  ++i){
    if(i==0){Ftot[i]=magU[k]*Fs[i]+S[1][i];  mag[i]=s.blend[i]*magU[k]+1.0-s.blend[i]; }
    if(i==1){Ftot[i]=magB[k]*Fs[i]+S[1][i];  mag[i]=s.blend[i]*magB[k]+1.0-s.blend[i]; }
    if(i==2){Ftot[i]=magV[k]*Fs[i]+S[1][i];  mag[i]=s.blend[i]*magV[k]+1.0-s.blend[i]; }
    if(i==3){Ftot[i]=magR[k]*Fs[i]+S[1][i];  mag[i]=s.blend[i]*magR[k]+1.0-s.blend[i]; }
    if(i==4){Ftot[i]=magI[k]*Fs[i]+S[1][i];  mag[i]=s.blend[i]*magI[k]+1.0-s.blend[i]; }
    Fbas[i] =        Fs[i] + S[0][i];
    magt[i]=s.blend[i]*(Ftot[i]/Fbas[i])+1.0-s.blend[i];}
   
    
    delVIl=  -2.5*log10(double(mag[2])/double(mag[4]))*1000.0;
    delBRl=  -2.5*log10(double(mag[1])/double(mag[3]))*1000.0; 
    delVIls= -2.5*log10(double(magt[2])/double(magt[4]))*1000.0;
    delBRls= -2.5*log10(double(magt[1])/double(magt[3]))*1000.0;
    cade= 40.0*sqrt(3.0/sqrt(5.0)/fabs(magt[4]))/60.0;///hour    
    ...
    MV= s.magb[2]-2.5*log10( magt[2] );
    MI= s.magb[4]-2.5*log10( magt[4] ); 
    MRR=s.magb[3]-2.5*log10( magt[3] );  
    weather=fabs((double)rand()*100.0/(double)(RAND_MAX+1.));
    if(magt[4]>Amax)   Amax=fabs(magt[4]); 
    ...
    col= col0+ ttr* sigc;
    deltaA= fabs(pow(10.0,-0.4*er.errI)-1.0)*magt[4]; ///error in magnification
    chi0 += (col- col0 )*(col- col0)/(sigc*sigc);
    chi1 += (col- s.col)*(col-s.col)/(sigc*sigc); 
    if(flagff>0)fprintf(danish,"%.5lf %.5lf  %.5lf  %.5lf  %.5lf\n",
    (t-l.t0)/l.tstar,double(col-s.col)*1000.0,double(magt[4]+ttr*deltaA),sigc*1000.0,deltaA);
    ndat+=1;}    
    if(tim>cade) tim = tim-cade;
    if(flagff>0) fprintf(magni,"%.6lf  %.6lf  %.6lf %.5lf %.5lf  %.5lf %.5lf  %.5lf   %.5lf   %.5lf  %.5lf  %.5lf  %.5lf %.6lf %.6lf %.5lf %.5lf\n", (t-l.t0)/l.tstar, xlens, ylens, mag[0],mag[1],mag[2],mag[3],mag[4],magt[0],magt[1],magt[2], magt[3], 
   magt[4],delVIl,delBRl, delVIls, delBRls); ///17
  
    if(fabs(delVIls)>1.0 and ftime==0) {interval=t;    ftime=1; }
    if((fabs(delVIls)<1.0 or k==int(Nlens-1)) and ftime==1){deltt+=double(t-interval);  ftime=0;}
    if(cVI<fabs(delVIls))  cVI= fabs(delVIls); 
    if(cBR<fabs(delBRls))  cBR= fabs(delBRls); 
    
   if(k%3000==0){
   cout<<"t:  "<<t<<"\t xlens:  "<<xlens<<"\t ylens:  "<<ylens<<endl;
   cout<<"delVI_limb:  "<<delVIl<<"\t delBR_limb:  "<<delBRl<<endl;
   cout<<"delVI_limb_spot :  "<<delVIls<<"\t delBR_limbspot:  "<<delBRls<<endl;
   cout<<"k:  "<<k<<"\t Nlens:  "<<Nlens<<endl;
   cout<<"magt[0]: "<<magt[0]<<"\t magt[1]:  "<<magt[1]<<"\t magt[2]:  "<<magt[2]<<endl;
   cout<<"magt[V-band]: "<<magt[2]<<"\t magt[I-band]:  "<<magt[4]<<endl;
   cout<<"*******************************************************"<<endl;}
   }
   if(flagff>0) {fclose(magni);   fclose(spots);   fclose(danish);  }
   dchi=fabs(chi0-chi1); 

   param=fopen("./files/MONTSpotB/SpotChroB_all_a.txt","a+");
   fprintf(param,"%d  %d  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.5lf  %.7lf   %.5lf  %.5lf  %d  %d  %d  %.5lf  %.5lf  %.5lf %.5lf   %.5lf  %.5lf  %.8lf  %.5lf  %.5lf   %.5lf  %.5lf  %.5lf   %.5lf  %d   %.5lf  %.5lf  %d  %.5lf  %.5lf %.5lf  %.5lf %.5lf %.5lf   %.5lf\n",
   ...
   p.tet*RA,p.fi*RA,p.tet0*RA,p.frac,icon, dchi, s.blend[4], ndat, l.q, l.dis, s.slop, deltt, cVI, cBR, Amax);//40
   fclose(param);
    ...
    cout<<"magb[I-band]:  "<<s.magb[4]<<"\t map[I-band]: "<<s.Map[4]<<endl;
    cout<<"Rstar[Rsun]: "<<s.Rs<<"\t Teff[K]:  "<<s.Teff<<endl;
    cout<<"ro_star: "<<s.ro_star<<"\t u0: "<<l.u0/s.ro_star<<endl;
    cout<<"logg:  "<<s.logg<<"\t logl:  "<<s.logl<<"\t s.Teff:  "<<s.Teff<<endl;
    cout<<"SPOT tet: "<<p.tet*RA<<"\t fi:  "<<p.fi*RA<<"\t tet0:  "<<p.tet0*RA<<endl;
    cout<<"frac:  "<<p.frac<<"\t dchi:  "<<dchi<<"\t ndat:  "<<ndat<<endl;
    cout<<"***************************************************** "<<endl;
...
void CauCross(lens & l, int icon){
   ...
   FILE *caustic;
   char filnam1[40];
   sprintf(filnam1,"./files/MONTSpotB/%c%c%c%c%d.dat",'c','a','u','_',icon);
   caustic=fopen(filnam1,"r");
   if(!caustic) {cout<<"File is not exit!!!! "<<"\t icoc:  "<<icon<<endl;}
   double xcau[800], ycau[800];
  // double xlens, ylens, dis;
  // double tcross, cross=9745602.043865; 
  // int N=int((2.6*l.tE)/l.dt +5.0); 
  // cout<<"N:    "<<N<<endl;
  // double *dism=new double[N];
  // double *t=new double[N];
  // cout<<"The array is made !!!   "<<endl;
   ...
   if(rad<0 or rad>799) {cout<<"Error:  "<<rad<<endl;   int uuw; cin>>uuw; }
 ...
 /*  for(int i=0; i<N; ++i){
   t[i]=double( -l.tE*1.3 + i*l.dt ); 
   xlens = (t[i]-l.t0)/l.tE * cos(l.ksi) - l.u0 * sin(l.ksi);
   ylens = (t[i]-l.t0)/l.tE * sin(l.ksi) + l.u0 * cos(l.ksi);
   dism[i]=1465745.746524;
   for(int j=0; j<800; ++j){
   dis=sqrt( (xlens-xcau[j])*(xlens-xcau[j])+(ylens-ycau[j])*(ylens-ycau[j]) ); 
   if(dis<dism[i])  dism[i]=dis;}}
   //cout<<"t:  "<<t[i]<<"\t   dism[i]: "<<dism[i]<<endl;
   for(int i=0; i<N; ++i) {
   if(cross>dism[i]){cross=double(dism[i]);  tcross=t[i];}}
   cout<<"cross:  "<<cross<<"\t tcross:  "<<tcross <<endl;
   l.pt1=double( tcross - 24.5/24.0); 
   l.pt2=double( tcross + 24.5/24.0); */
...
///XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
void Func_spot(spot & p){
     p.tet0= double(((double)rand()/(double)(RAND_MAX+1.0))*5.0+15.0)/RA;
     p.frac= double( (double)rand()/(double)(RAND_MAX+1.0) )*0.6+0.1;
     p.tet= (double( (double)rand()/(double)(RAND_MAX+1.0) )*88.0+1.0)/RA;
     p.fi=  (double( (double)rand()/(double)(RAND_MAX+1.0) )*88.0+1.0)/RA;
}
///XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
///                                                                ///
///                        Func_source                             ///
...
void func_source(source & s, CMD & cm, extinc & ex)
{
   ...
   double Mab[M]; 
    ...
    if(s.type>8.0 or s.type<2.0 or s.Rs<0.0 or s.Rs>1000.0 or s.mass<0.0 or s.Teff<0.0 or s.cl==6 or s.logg<0.0 or 
    s.cl<0  or s.cl>9 or s.slop<1.5 or s.slop>2.5){
